#
# (C) 2021 zoidtechnologies.com All Rights Reserved.
#
...
def _edit(args, record, mode="edit"):
    ...
    while not done:
        ...
        if ch == "Q":
            ...
            ttyio.echo("Name")
            editrecord["name"] = ttyio.inputstring(
                "fooditem name: ", editrecord["name"]
            )
        elif ch == "T":
            ttyio.echo("Title")
            editrecord["title"] = ttyio.inputstring(
                "fooditem title: ", editrecord["title"]
            )
    return editrecord
...
def setarea(args, left, stack=False):
    def right():
        ...
        rightbuf = "| %s | %s" % (
            currentmember["name"],
            bbsengine.pluralize(currentmember["credits"], "credit", "credits"),
        )
...
def main(args):
    ...
    while not done:
        ...
        buf = ""
        for m in mainmenu:
            buf += "{bgdarkgray}{white}[{yellow}%s{white}]{/all} -- %s{F6}" % (
                m[0],
                m[1],
            )
